...
@api_view(['GET', 'POST'])
def encode_steganography(request):
    if request.method == 'POST':
        cover_image_path = request.data['cover_image_path']
        secret_image_path = request.data['secret_image_path']
        host = HostElement(cover_image_path) #Full path of the cover image
        password = filename = None  #Default value is None since we're not implementing password
        filename = secret_image_path #Full path of the secret image
        with open(filename, 'rb') as myfile:
                    message = myfile.read()
        host.insert_message(message, 2, filename, password)
        host.save()
        # host = HostElement('_'+cover_image_path)
        # host.read_message()

        return Response(
           data={
               "message": "Steganography Image Applied",                
               "steganography_image": 'http://localhost:8000/media/_'+cover_image_path,
            #    "secret_image": "http://localhost:8000/media/_secret.png"
               },
               status=status.HTTP_201_CREATED
           )
@api_view(['GET', 'POST'])
def decode_steganography(request):
    # if request.method == 'POST':
          steganography_image_path = request.data['steganography_image_path']
          print(steganography_image_path)
          host = HostElement(steganography_image_path)
          host.read_message()
        #   print(host.data.data)
          os.replace("_images/_secret.png", "images/secret.png")
          return Response(
           data={
               "message": "Secret Image is here",    
               "secret_image": "http://localhost:8000/media/images/secret.png"
               },
               status=status.HTTP_201_CREATED
           )
...
class ImageViewSet(viewsets.ModelViewSet):
    ...
    queryset = ImageName.objects.all()
    ...
    def create(self, request, *args, **kwargs):        
        ...
        return Response(
           data={
               ...
               "data": serializer.data['image'].replace('http://localhost:8000/media/','')           
               },
               status=status.HTTP_201_CREATED,
               headers=headers
           )
